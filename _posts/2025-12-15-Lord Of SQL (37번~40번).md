...
이렇게 할 수 있는 이유는 SQLite는 데이터베이스 자체가 자신의 구조를 설명할 수 있게 설계되어 있기 때문이다.<br>
```sql
-- sqlite_master 구조
